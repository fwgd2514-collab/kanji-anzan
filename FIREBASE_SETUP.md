...
この設定を行うと、GitHub Pagesの公開方法はそのままで、名前ごとのレベル・XP・連続日数をFirestoreへ保存できます。
...
1. Firebase左側の「構築」から「Firestore Database」を開きます。
2. 「データベースの作成」を選びます。
3. 利用する地域に近いロケーションを選びます。
4. 作成後、「ルール」タブを開きます。
...
別の端末でも同じGitHub PagesのURLを開くと、同じ `groupId` のレベル一覧を取得します。
